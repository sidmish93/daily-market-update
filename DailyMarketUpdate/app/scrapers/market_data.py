...
def _fmt_rs_crore(value: float) -> str:
    return f"Rs {value:,.2f} crore"


def _net_flow_phrase(net: float) -> str:
    amount = _fmt_rs_crore(abs(net))
    if net > 0:
        return f"net buyers of {amount}"
    if net < 0:
        return f"net sellers of {amount}"
    return f"flat at {amount}"


def fetch_fii_dii_capital_market_news() -> "NewsUpdateItem | None":
    """
    Latest FII/FPI & DII trading activity on NSE, BSE and MSEI
    in the Capital Market Segment (NSE reports page).
    """
    from app.models import NewsUpdateItem
    ...
    try:
        with httpx.Client(timeout=20.0, headers=NSE_HEADERS, follow_redirects=True) as client:
            ...
            resp = client.get(
                url,
                headers={**NSE_HEADERS, "Referer": "https://www.nseindia.com/reports/fii-dii"},
            )
            resp.raise_for_status()
            rows = resp.json()
    except Exception as exc:  # noqa: BLE001
        logger.warning("NSE FII/DII flows fetch failed: %s", exc)
        return None
    ...
    fii = by_cat.get("FII/FPI")
    dii = by_cat.get("DII")
    if not fii or not dii:
        logger.warning("NSE FII/DII payload missing categories: %s", rows)
        return None

    try:
        fii_net = float(str(fii.get("netValue")).replace(",", ""))
        dii_net = float(str(dii.get("netValue")).replace(",", ""))
    except (TypeError, ValueError):
        logger.warning("NSE FII/DII net values unparsable: %s", rows)
        return None

    date_raw = str(fii.get("date") or dii.get("date") or "").strip()
    date_label = date_raw.replace("-", " ") if date_raw else "the latest session"

    text = (
        f"Fund flows: On {date_label}, FIIs/FPIs were {_net_flow_phrase(fii_net)} "
        f"in the capital market across NSE, BSE and MSEI, while DIIs were "
        f"{_net_flow_phrase(dii_net)}."
    )
    return NewsUpdateItem(text=text, source="NSE")
