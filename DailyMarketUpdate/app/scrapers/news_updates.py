...
def _is_competing_fii_dii_flow(text: str) -> bool:
    low = (text or "").lower()
    if not re.search(r"\b(fii|fpi|diis?|foreign (?:fund|institutional)|domestic institutional)\b", low):
        return False
    return bool(
        re.search(
            r"\b(net (?:buy|sell|buyer|seller|bought|sold)|fund flows?|bought|sold|"
            r"inflow|outflow|capital market)\b",
            low,
        )
    )
...
def select_news_updates(
    ...
    movers: list | None = None,
    focus_rows: list[StockFocusRow] | None = None,
    forced_items: list[NewsUpdateItem] | None = None,
) -> list[NewsUpdateItem]:
    """Pick distinct bullets; reserve room for macro + stock/sector catalysts."""
    forced = [i for i in (forced_items or []) if i and (i.text or "").strip()]
    ranked = sorted(
    ...
    for item in forced:
        cluster = _story_cluster_key(item.text)
        key = re.sub(r"[^a-z0-9]+", "", item.text.lower())[:80]
        if cluster in seen_clusters or key in seen_text:
            continue
        seen_clusters.add(cluster)
        seen_text.add(key)
        out.append(item)

    drop_flow_dupes = any(_is_competing_fii_dii_flow(i.text) for i in forced)

    # Up to 2 macro / geopolitics / policy / flows bullets first
    for item in (n for n in pool if _is_macro(n)):
        if len(out) >= min(2 + len(forced), max_items):
            break
        if drop_flow_dupes and _is_competing_fii_dii_flow(f"{item.title} {item.summary}"):
            continue
        _try_append(
