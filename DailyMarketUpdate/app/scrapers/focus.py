...
def _polish_focus_snippet(snippet: str, item: NewsCandidate) -> str:
    ...
    # Prefer precise tenure language for the Chandrasekaran story
    if re.search(r"chandrasekaran|n chandra", snippet, re.I):
        snippet = re.sub(
        ...
        snippet = re.sub(
            r",?\s*wipe out Rs?\s*[\d,]+\s*crore\b",
            "",
            snippet,
            flags=re.I,
        ).strip(" ,")
    # Drop soft editorial tails
    snippet = re.sub(
        r"[.?]?\s*What should investors do\??\.?$",
        ".",
        snippet,
        flags=re.I,
    )
