...
from app.scrapers.market_data import (
    ...
    fetch_fii_dii_capital_market_news,
...
async def generate_report_draft(
    ...
    async def track(name: str, coro) -> Any:
        try:
            ...
            if result is None:
                ...
                status[name] = "ok" if result is not None else "empty"
    ...
    indian, globals_, sectors, movers, nifty200, ad, commodities, fii_dii = await asyncio.gather(
        ...
        track("fii_dii", _run_sync(fetch_fii_dii_capital_market_news)),
    )
    ...
    if include_narrative:
        try:
            forced_news = [fii_dii] if fii_dii is not None else []
            narrative, focus = await generate_narrative(
                draft,
                forced_news_updates=forced_news,
            )
